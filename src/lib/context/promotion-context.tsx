...
export const PromotionContext = createContext<PromotionContextType | undefined>(
  undefined,
);
...
export function PromotionProvider({ children }: { children: ReactNode }) {
  ...
  const [promotions, setPromotions] = useLocalStorage<Promotion[]>(
    "mali-gamepass-promotions",
    [
      {
        id: "promo-001",
        title: "Flash Sale: 30% Off PUBG Mobile UC",
        description:
          "Get 30% off all PUBG Mobile UC purchases for the next 24 hours!",
        type: "flash_sale",
        ...
        isActive: true,
        bannerImage:
          "https://placehold.co/1200x300/003366/ffffff?text=PUBG+Flash+Sale",
      ...
      },
      {
        ...
        isActive: true,
        bannerImage:
          "https://placehold.co/1200x300/000000/ffffff?text=Steam+Cashback",
        discountType: "percentage",
        discountValue: 10,
        minPurchase: 10,
        maxDiscount: 50,
        ...
        terms:
          "Cashback will be credited to your account within 24 hours of purchase. Maximum cashback amount is $50.",
      },
      {
        id: "promo-003",
        title: "Refer a Friend, Get $5",
        description:
          "Refer a friend and both get $5 credit when they make their first purchase",
        type: "referral",
        ...
        isActive: true,
        bannerImage:
          "https://placehold.co/1200x300/6633cc/ffffff?text=Refer+A+Friend",
        discountType: "fixed_amount",
        discountValue: 5,
        terms:
          "Your friend must make a purchase of at least $20 for both of you to receive the reward.",
      },
      {
        id: "promo-004",
        ...
        isActive: false,
        bannerImage:
          "https://placehold.co/1200x300/990000/ffffff?text=Free+Shipping",
        discountType: "percentage",
        discountValue: 100,
  ...
  const [userPromotions, setUserPromotions] = useLocalStorage<UserPromotion[]>(
    "mali-gamepass-user-promotions",
    [
      {
        id: "user-promo-001",
        userId: userId,
  ...
  // Mock cashbacks
  const [cashbacks, setCashbacks] = useLocalStorage<Cashback[]>(
    "mali-gamepass-cashbacks",
    [
      {
        id: "cashback-001",
        userId: userId,
  ...
  const getActivePromotions = (): Promotion[] => {
    ...
    return promotions.filter(
      (promo) =>
        promo.isActive && promo.startDate <= now && promo.endDate >= now,
    );
  };
  ...
  const claimPromotion = async (promotionId: string): Promise<boolean> => {
    ...
    try {
      ...
      const existingClaim = userPromotions.find(
        (up) => up.userId === userId && up.promotionId === promotionId,
      );

      if (existingClaim && existingClaim.claimed) {
        if (
          promotion.limitPerUser &&
          existingClaim.usageCount >= promotion.limitPerUser
        ) {
          throw new Error(
      ...
      setError(
        error instanceof Error ? error.message : "Failed to claim promotion",
      );
      return false;
    } finally {
      setIsLoading(false);
  ...
  const applyPromotion = async (
    ...
    try {
      ...
      const userPromotion = userPromotions.find(
        (up) =>
          up.userId === userId && up.promotionId === promotionId && up.claimed,
      );
      ...
      if (
        promotion.limitPerUser &&
        userPromotion.usageCount >= promotion.limitPerUser
      ) {
  ...
  const useCashback = async (amount: number): Promise<boolean> => {
    ...
    try {
      ...
      const totalAvailable = availableCashbacks.reduce(
        (total, cb) => total + cb.amount,
        0,
      );

      if (amount > totalAvailable) {
        throw new Error(
          `Insufficient cashback balance. Available: ${totalAvailable}`,
        );
      }
      ...
      availableCashbacks
        ...
        .forEach((cashback) => {
          ...
          const cashbackIndex = updatedCashbacks.findIndex(
            (cb) => cb.id === cashback.id,
          );
      ...
      setError(
        error instanceof Error ? error.message : "Failed to use cashback",
      );
      return false;
    } finally {
      setIsLoading(false);
  ...
  const generateReferralLink = (): string => {
    if (!referral) return "";
    return `https://lnwtermgame.com/refer/${referral.referralCode}`;
  };
