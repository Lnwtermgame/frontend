...
import { supportApi, FaqCategory, FaqArticle, FaqArticleListItem } from "@/lib/services";
...
export default function AdminFaqPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (articlesRes.success) {
        ...
        const articlesWithCategory: FaqArticleWithCategory[] = articlesRes.data.map((article: FaqArticleListItem) => {
          const cat = categoriesRes.data.find((c: FaqCategory) => c.id === article.categoryId);
          return {
            id: article.id,
  ...
  const handleUpdateCategory = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await supportApi.updateFaqCategory(editingCategory.id, payload);
      if (response.success) {
        setShowCategoryModal(false);
  ...
  const handleDeleteCategory = async (categoryId: string) => {
    if (!confirm("คุณแน่ใจหรือไม่ว่าต้องการลบหมวดหมู่นี้? บทความทั้งหมดในหมวดหมู่นี้จะถูกลบด้วย")) return;
  ...
  const handleUpdateArticle = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingArticle || !articleForm.title.trim() || !articleForm.content.trim()) return;

    setIsSubmitting(true);
    try {
      const response = await supportApi.updateFaqArticle(editingArticle.id, articleForm);
      if (response.success) {
        setShowArticleModal(false);
  ...
  return (
    <AdminLayout title="จัดการคำถามที่พบบ่อย (FAQ)">
      <div className="space-y-6">
        ...
        >
          {[
            { label: "หมวดหมู่", value: categories.length, color: "bg-blue-100 text-blue-700 border-blue-500" },
            { label: "บทความทั้งหมด", value: articles.length, color: "bg-green-100 text-green-700 border-green-500" },
            { label: "บทความที่ปักหมุด", value: articles.filter((a) => a.isPinned).length, color: "bg-yellow-100 text-yellow-700 border-yellow-500" },
            { label: "ไม่แสดง", value: articles.filter((a) => !a.isActive).length, color: "bg-gray-100 text-gray-700 border-gray-500" },
          ].map((stat, index) => (
            <div
              key={index}
                ...
                {categories.map((category) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center">
                        {category.icon && <span className="text-2xl mr-2">{category.icon}</span>}
                    ...
                    <p className="text-sm text-gray-600 mb-2">{category.description || "ไม่มีคำอธิบาย"}</p>
                    <div className="flex items-center justify-between text-xs text-gray-500">
                      <span>Slug: {category.slug}</span>
                      <span>{articles.filter((a) => a.categoryId === category.id).length} บทความ</span>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <div key={article.id} className="p-4 hover:bg-gray-50 transition-colors">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2">
                      {article.isPinned && <Pin size={16} className="text-brutal-yellow" />}
                      {!article.isActive && <AlertCircle size={16} className="text-gray-400" />}
                      <h3 className={`font-bold ${!article.isActive ? 'text-gray-400' : 'text-black'}`}>
                        {article.title}
                      </h3>
                    </div>
                    <div className="flex gap-1">
          ...
          {showCategoryModal && (
            ...
            >
              ...
              >
                ...
                >
                  <div>
                    <label className="block text-gray-700 mb-2 font-medium">ชื่อหมวดหมู่ *</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-gray-700 mb-2 font-medium">Slug (ไม่บังคับ)</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-gray-700 mb-2 font-medium">คำอธิบาย</label>
                    <textarea
                  ...
                  <div>
                    <label className="block text-gray-700 mb-2 font-medium">ไอคอน (emoji)</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-gray-700 mb-2 font-medium">ลำดับการแสดง</label>
                    <input
                      type="number"
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <div className="bg-brutal-pink/10 border-[2px] border-brutal-pink p-4">
                    ...
                    <AnimatePresence>
                      {showAIGenerate && (
                        ...
                        >
                          <div>
                            <label className="block text-gray-700 mb-1 text-sm">หัวข้อที่ต้องการให้ AI เขียน</label>
                            <input
                              type="text"
                              value={aiTopic}
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              onClick={async () => {
                                ...
                                try {
                                  const categoryName = categories.find(c => c.id === articleForm.categoryId)?.name || "ทั่วไป";
                                  const result = await aiService.generateFaqContent(
                                    aiTopic,
                                    categoryName,
                                    (progress) => {
                                      // Optional: show progress in console or UI
                                      console.log(`[AI] ${progress.stage}: ${progress.message}`);
                                    }
                                  );
                                  ...
                                  setError(err.message || "ไม่สามารถสร้างเนื้อหาด้วย AI ได้");
                                } finally {
                                  setIsGeneratingAI(false);
                                }
                              }}
                              disabled={isGeneratingAI || !aiTopic.trim() || !articleForm.categoryId}
                          ...
                          {isGeneratingAI && (
                            <p className="text-xs text-gray-600">AI กำลังสร้างเนื้อหา กรุณารอสักครู่...</p>
                          )}
                        </motion.div>
                      )}
                    </AnimatePresence>
                  ...
                  <div>
                    <label className="block text-gray-700 mb-2 font-medium">เนื้อหา *</label>
                    <textarea
                  ...
                  <div>
                    <label className="block text-gray-700 mb-2 font-medium">บทสรุป (excerpt)</label>
                    <textarea
                  ...
                  <div className="flex gap-3 pt-4">
                    ...
                    <button
                      type="submit"
                      disabled={isSubmitting || !articleForm.title.trim() || !articleForm.content.trim() || !articleForm.categoryId}
                      className="flex-1 py-2.5 px-4 bg-black text-white border-[3px] border-black disabled:opacity-50 disabled:cursor-not-allowed font-medium hover:bg-gray-800 transition-colors"
                      style={{ boxShadow: '4px 4px 0 0 #000000' }}
                    >
                      {isSubmitting ? (
