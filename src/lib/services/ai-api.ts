...
class AiService {
  ...
  private buildClassificationPrompt(
    ...
    return `คุณคือผู้เชี่ยวชาญด้านตลาดเกมในภูมิภาคเอเชียตะวันออกเฉียงใต้และตลาดสากล มีความรู้ลึกซึ้งเกี่ยวกับเกมทุกแพลตฟอร์ม จัดหมวดหมู่สินค้าเกมนี้และวิเคราะห์ศักยภาพทางการตลาด
...
จัดหมวดหมู่ตอนนี้ (ตอบเฉพาะรูปแบบด้านบน):`;
  ...
  private buildDescriptionPrompt(
    ...
    return `คุณคือ Copywriter ระดับมืออาชีพสำหรับเว็บไซต์เติมเกม "Lnwtermgame" เขียนคำอธิบายสินค้าที่ละเอียด น่าเชื่อถือ ช่วยให้ลูกค้าตัดสินใจซื้อได้ง่าย
...
เขียนคำอธิบายตอนนี้:`;
  ...
  private buildShortDescriptionPrompt(
    ...
    return `คุณคือ Copywriter ระดับมืออาชีพที่เชี่ยวชาญด้าน Conversion Copywriting สร้างคำอธิบายสั้นที่กระชับ ดึงดูด และกระตุ้นให้ลูกค้าตัดสินใจซื้อ
...
${fullDescription.substring(0, 500)}...
  ...
  private buildMetaPrompt(
    ...
    return `คุณคือ SEO Specialist ระดับมืออาชีพที่เชี่ยวชาญการเขียน Meta Tags สำหรับ E - commerce เว็บไซต์เติมเกม สร้าง Meta Tags ที่ช่วยเพิ่มอันดับ Google และเพิ่ม CTR
...
สร้าง Meta Tags ตอนนี้(ตอบเฉพาะรูปแบบด้านบน): `;
  ...
  private buildGameDetailsPrompt(
    ...
    return `คุณคือผู้เชี่ยวชาญด้านอุตสาหกรรมเกมที่มีความรู้ครอบคลุมทุกแพลตฟอร์ม ทั้ง Mobile, PC, Console และ Cross - Platform ระบุข้อมูลเกมให้ถูกต้องตามความเป็นจริง
...
${existingSlugs && existingSlugs.length > 0 ? `ห้ามใช้ slug เหล่านี้ซ้ำ: ${existingSlugs.slice(0, 10).join(", ")}` : ""}`;
  ...
  private buildFaqPrompt(topic: string, categoryName: string): string {
    return `คุณคือผู้เชี่ยวชาญด้านการเขียนบทความ FAQ สำหรับเว็บไซต์เติมเกมและบัตรดิจิทัล "Lnwtermgame" คุณต้องเขียนคำตอบที่ละเอียด ครบถ้วน เป็นประโยชน์สูงสุดต่อผู้อ่าน
...
สร้างบทความ FAQ ตอนนี้ (ตอบเฉพาะรูปแบบด้านบน):`;
  ...
  private buildNewsPrompt(topic: string, categoryName: string): string {
    return `คุณคือนักข่าวเกมมืออาชีพระดับสูง เขียนบทความข่าวสารเชิงลึกสำหรับเว็บไซต์เติมเกม "Lnwtermgame" คุณต้องเขียนข่าวที่ละเอียด ครบถ้วน มีโครงสร้างเป็นระบบ และมีคุณภาพเทียบเท่าสำนักข่าวเกมชั้นนำ
...
- เผยข้อมูลที่หลายคนไม่รู้`,
  ...
  private buildCmsPagePrompt(topic: string, pageType: string): string {
    return `คุณคือนักเขียนเนื้อหากฎหมายและนโยบายเว็บไซต์ระดับมืออาชีพ เชี่ยวชาญด้านการเขียนหน้า CMS สำหรับธุรกิจ E-commerce ในประเทศไทย คุณต้องเขียนเนื้อหาให้ครบถ้วน ละเอียด มีโครงสร้างเป็นระบบ และมีคุณภาพเทียบเท่าเว็บไซต์ E-commerce ชั้นนำ

ชื่อหน้า: "${topic}"
ประเภทหน้า: ${pageType}
บริบท: เว็บไซต์จำหน่ายบัตรเติมเกม, เติมเกมออนไลน์, และบัตรดิจิทัลต่างๆ (Gift Card) ชื่อ "Lnwtermgame" สำหรับตลาดไทยและสากล
