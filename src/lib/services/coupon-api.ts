...
class CouponApiService {
  async getAvailableCoupons(
    ...
    const response = await orderClient.get<CouponsListResponse>(
      `/api/coupons?${params}`,
      { signal },
    );
  ...
  async getMyCoupons(
    ...
    limit = 20,
    signal?: AbortSignal,
  ): Promise<UserCouponsListResponse> {
    const params = new URLSearchParams();
    params.append("page", String(page));
    params.append("limit", String(limit));

    const response = await orderClient.get<UserCouponsListResponse>(
      `/api/coupons/my?${params}`,
      { signal },
    );
    return response.data;
  }
  ...
  async claimCoupon(couponId: string): Promise<CouponResponse> {
    const response = await orderClient.post<CouponResponse>(
      `/api/coupons/${couponId}/claim`,
    );
    return response.data;
  }
  ...
  async validateCoupon(
    ...
    const response = await orderClient.post<ValidateCouponResponse>(
