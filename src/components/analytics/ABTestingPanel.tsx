...
export default function ABTestingPanel({ initialTests = [] }: ABTestingPanelProps) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "active":
        return <span className="px-3 py-1 text-xs bg-green-100 border-2 border-black text-green-700 font-medium">Active</span>;
      case "completed":
        return <span className="px-3 py-1 text-xs bg-blue-100 border-2 border-black text-blue-700 font-medium">Completed</span>;
      case "paused":
        return <span className="px-3 py-1 text-xs bg-yellow-100 border-2 border-black text-yellow-700 font-medium">Paused</span>;
      default:
    ...
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex bg-white border-2 border-black overflow-hidden mr-2" style={{ boxShadow: '2px 2px 0 0 #000000' }}>
            {[
      ...
      {showCreateForm && (
        ...
        >
          <div className="flex justify-between items-start mb-4">
            <h4 className="text-lg font-bold text-black">Create New A/B Test</h4>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-sm text-gray-700 mb-1.5 font-medium">Test Name</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm text-gray-700 mb-1.5 font-medium">Target Page</label>
              <input
                type="text"
                className="w-full bg-white border-2 border-black px-3 py-2 text-black focus:outline-none focus:ring-2 focus:ring-brutal-blue"
            ...
            <div>
              <label className="block text-sm text-gray-700 mb-1.5 font-medium">Test Duration</label>
              <select
                className="w-full bg-white border-2 border-black px-3 py-2 text-black focus:outline-none focus:ring-2 focus:ring-brutal-blue"
              >
          ...
          <div className="mb-4">
            <label className="block text-sm text-gray-700 mb-1.5 font-medium">Variants</label>
      ...
      {filteredTests.length === 0 ? (
        <div className="bg-white border-[3px] border-black p-8 text-center" style={{ boxShadow: '4px 4px 0 0 #000000' }}>
          ...
          <h3 className="text-lg font-bold text-black mb-2">No {selectedTab} tests found</h3>
            ...
            >
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <span className={`text-sm font-bold ${isWinner ? "text-green-700" : "text-black"}`}>
                          {variant.name}
                          {isWinner && <Check className="inline-block h-3.5 w-3.5 ml-1.5 text-green-600" />}
                        </span>
                        <span className="text-xs text-gray-600">{variant.visitors.toLocaleString()} visitors</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-gray-600">Conversion: {variant.conversion}%</span>
                        <span className={`text-xs font-medium ${variant.change > 0 ? 'text-green-600' : variant.change < 0 ? 'text-red-600' : 'text-gray-500'}`}>
              ...
              {test.status === "active" && (
                <div className="flex gap-2 mt-3">
                  ...
                  <button className="px-3 py-1.5 text-xs bg-gray-100 border-2 border-black text-gray-700 hover:bg-gray-200 font-medium transition-colors"
                    style={{ boxShadow: '2px 2px 0 0 #000000' }}
                  >
                    Stop Test
                  </button>
              ...
              {test.status === "completed" && (
                <div className="flex gap-2 mt-3">
                  ...
                  <button className="px-3 py-1.5 text-xs bg-green-100 border-2 border-black text-green-700 hover:bg-green-200 flex items-center font-medium transition-colors"
                    style={{ boxShadow: '2px 2px 0 0 #000000' }}
                  >
