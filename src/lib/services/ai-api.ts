...
${inferredPlatforms ? `\nคำแนะนำ: สินค้านี้อยู่ในหมวดหมู่ "${categoryName}" ซึ่งปกติจะรองรับแพลตฟอร์ม: ${inferredPlatforms} (ใช้เป็นแนวทาง แต่ให้ตรวจสอบจากชื่อสินค้าด้วย)` : ""}
  ...
  async generateNewsContent(
    ...
    newsProvider: "thenewsapi" | "newsapi" = "thenewsapi",
  ): Promise<GeneratedEditorialContent> {
    const startTime = Date.now();
    console.log("[News] ========== GENERATE NEWS CONTENT START ==========");
    console.log("[News] Parameters:", { topic, categoryName, variation: variation || "(none)", newsProvider, existingSlugsCount: existingSlugs?.length || 0 });
    ...
    try {
      ...
      const providerLabel = newsProvider === "newsapi" ? "NewsAPI" : "TheNewsAPI";
      onProgress?.({
        stage: "preparing",
        message: `กำลังค้นหาข่าวจาก ${providerLabel}...`,
      });

      // Step 2: Fetch reference articles from the selected news provider
      let apiEndpoint: string;
      let apiParams: Record<string, string>;

      if (newsProvider === "newsapi") {
        // NewsAPI.org
        apiEndpoint = "/api/newsapi-search";
        apiParams = { q: topic, limit: "5" };
      } else {
        // TheNewsAPI (default)
        ...
        const apiCategory = categoryMap[categoryName] || "tech";
        apiEndpoint = "/api/news-search";
        apiParams = { q: topic, category: apiCategory, limit: "5" };
      }

      console.log(`[News] Calling ${apiEndpoint} with:`, apiParams);
      ...
      console.log(`[News] Received ${articles.length} articles from ${providerLabel}`);

      if (articles.length === 0) {
        throw new Error(`ไม่พบข่าวจาก ${providerLabel} สำหรับหัวข้อนี้ ลองเปลี่ยนคำค้นหาหรือเปลี่ยนแหล่งข่าว`);
      }

      onProgress?.({
        stage: "searching",
        message: `พบ ${articles.length} ข่าว กำลังส่งให้ AI เขียนเนื้อหา...`,
      });

      // Step 3: Build prompt with articles as reference and send to LiteLLM
      const newsPrompt = this.buildNewsPrompt(topic, categoryName, articles, variation);

      console.log("[News] Sending articles to LiteLLM for content generation...");

      onProgress?.({
        stage: "parsing",
        message: "AI กำลังเขียนบทความข่าว...",
      });

      const aiResponse = await this.callLiteLLMApiCustom(
        `${BUSINESS_DNA}

คุณคือนักข่าวเกมมืออาชีพของ Lnwtermgame เชี่ยวชาญในการเขียนข่าวเกมและเทคโนโลยีภาษาไทย
คุณจะได้รับข้อมูลข่าวจากแหล่งต่างๆ เป็น reference และต้องเขียนบทความข่าวใหม่ที่สมบูรณ์ น่าสนใจ น่าอ่าน
ห้ามคัดลอกข้อมูลจาก reference ตรงๆ ต้องเรียบเรียงใหม่ทั้งหมดเป็นภาษาไทย
ตอบเฉพาะรูปแบบที่กำหนดเท่านั้น ห้ามมีข้อความอื่นนอกรูปแบบ`,
        newsPrompt,
      );

      const choice = aiResponse.choices[0];
      let aiContent = choice.message.content?.trim() || "";
      ...
      if (!aiContent && reasoning) {
        ...
        if (contentMatch) {
          aiContent = contentMatch[1].trim();
        }
      }

      if (!aiContent) {
        throw new Error("AI ไม่สามารถสร้างเนื้อหาได้ กรุณาลองใหม่อีกครั้ง");
      }

      console.log("[News] AI content received, length:", aiContent.length);

      // Step 4: Parse the structured AI response
      const result = await this.parseEditorialContent(aiContent, topic);

      // Override slug if needed
      if (!result.slug) {
        result.slug = slugify(result.title || topic);
      }
      if (existingSlugs?.includes(result.slug)) {
        result.slug = `${result.slug}-${Date.now().toString(36).slice(-4)}`;
      }
      ...
      const articleSources = articles
        ...
        .filter(Boolean)
        .slice(0, 3);
      result.sources = [...new Set([...(result.sources || []), ...articleSources])]
  ...
  /**
   * Build a comprehensive news writing prompt for LiteLLM using TheNewsAPI articles as reference
   */
  private buildNewsPrompt(
    topic: string,
    categoryName: string,
    articles: Array<{ title: string; description: string; snippet: string; source: string; published_at: string; url: string }>,
    variation?: string,
  ): string {
    // Build reference articles section
    const articleRefs = articles
      .map((article, i) => {
        const parts = [`### ข่าวอ้างอิง ${i + 1}`];
        if (article.title) parts.push(`หัวข้อ: ${article.title}`);
        if (article.description) parts.push(`เนื้อหา: ${article.description}`);
        if (article.snippet && article.snippet !== article.description) {
          parts.push(`รายละเอียด: ${article.snippet}`);
        }
        if (article.source) parts.push(`แหล่งที่มา: ${article.source}`);
        if (article.published_at) parts.push(`วันที่: ${article.published_at}`);
        if (article.url) parts.push(`URL: ${article.url}`);
        return parts.join("\n");
      })
      .join("\n\n");

    // Add variation directive if specified
    const variationLine = variation
      ? `\nแนวการเขียน: ${variation} — ให้เขียนบทความตามแนวทางนี้เป็นหลัก\n`
      : "";

    return `หัวข้อที่ต้องการเขียนข่าว: "${topic}"
หมวดหมู่: ${categoryName}${variationLine}

## ข้อมูลอ้างอิงจากแหล่งข่าว (TheNewsAPI)

${articleRefs}

## คำสั่ง

จากข้อมูลข่าวอ้างอิงด้านบน ให้เขียนบทความข่าวใหม่ที่สมบูรณ์เป็นภาษาไทย โดยปฏิบัติตามกฎดังนี้:

1. ข้อมูลต้องอ้างอิงจากข่าวด้านบนเป็นหลัก แต่ต้องเรียบเรียงใหม่ทั้งหมด ห้ามคัดลอกตรงๆ
2. เขียนด้วยภาษาไทยที่อ่านง่าย น่าสนใจ เหมาะกับกลุ่มเป้าหมายเกมเมอร์ไทย
3. เนื้อหาต้องยาวอย่างน้อย 800 คำ มีโครงสร้างชัดเจน
4. ใช้หัวข้อย่อย (## ) อย่างน้อย 4-6 หัวข้อ
5. ใช้ **ตัวหนา** เน้นชื่อเกม ชื่อตัวละคร ข้อมูลสำคัญ
6. ห้ามใส่อิโมจิในเนื้อหา
7. เพิ่มหัวข้อ "## สรุป" ตอนท้ายบทความ
8. สร้าง tags ที่เกี่ยวข้องอย่างน้อย 3-5 tags (เป็นภาษาไทยหรืออังกฤษก็ได้)

## รูปแบบการตอบ (ตอบเฉพาะรูปแบบนี้เท่านั้น)

TITLE: [หัวข้อข่าวภาษาไทยที่น่าสนใจ ไม่เกิน 100 ตัวอักษร]
CONTENT: [เนื้อหาบทความข่าวเต็ม ใช้ markdown สำหรับหัวข้อย่อย ตัวหนา และรายการ]
EXCERPT: [สรุปสั้นๆ ไม่เกิน 150 ตัวอักษร สำหรับ preview]
SLUG: [slug ภาษาอังกฤษ ใช้ตัวพิมพ์เล็กและขีดกลางเท่านั้น]
TAGS: [tag1, tag2, tag3, tag4, tag5]
SOURCES: [${articles.map((a) => a.url).filter(Boolean).join(", ")}]

สร้างบทความข่าวตอนนี้:`;
  }

  /**
   * Fetch news articles from TheNewsAPI (public method for UI)
   */
  async fetchNewsArticles(
    ...
    try {
      const response = await axios.get("/api/news-search", {
        params: { q: topic, category: category || "", limit: limit || 5 },
        timeout: 30000,
      });
      return response.data?.articles || [];
    } catch (error: any) {
      console.error("[News] fetchNewsArticles error:", error.message);
      return [];
    }
  }
...
สร้างเนื้อหาหน้า CMS ตอนนี้ (ตอบเฉพาะรูปแบบด้านบน):`;
  ...
  private async parseEditorialContent(
    ...
    for (const line of lines) {
      // Normalize: strip bold markers (**) and leading # for matching
      const normalizedLine = line.replace(/^\*\*\s*/, "").replace(/\*\*\s*$/, "").replace(/^\*\*/, "").replace(/\*\*$/, "");
      const upperLine = normalizedLine.toUpperCase();

      if (upperLine.startsWith("TITLE:")) {
        title = normalizedLine.replace(/^TITLE:\s*/i, "").trim();
        currentSection = null;
      } else if (upperLine.startsWith("CONTENT:")) {
        currentSection = "content";
        contentBuffer = [];
      } else if (upperLine.startsWith("EXCERPT:")) {
        if (currentSection === "content" && contentBuffer.length > 0) {
          articleContent = contentBuffer.join("\n").trim();
        }
        excerpt = normalizedLine.replace(/^EXCERPT:\s*/i, "").trim();
        currentSection = null;
      } else if (upperLine.startsWith("SLUG:")) {
        if (currentSection === "content" && contentBuffer.length > 0) {
          articleContent = contentBuffer.join("\n").trim();
        }
        slug = normalizedLine.replace(/^SLUG:\s*/i, "").trim();
        currentSection = null;
      } else if (upperLine.startsWith("TAGS:")) {
        if (currentSection === "content" && contentBuffer.length > 0) {
          articleContent = contentBuffer.join("\n").trim();
        }
        let tagsStr = normalizedLine.replace(/^TAGS:\s*/i, "").trim();
        // Handle inline SOURCES: within TAGS line (e.g. "VCT, SOURCES: url1, url2")
        const inlineSourcesIdx = tagsStr.toUpperCase().indexOf("SOURCES:");
        if (inlineSourcesIdx !== -1) {
          const sourcesInline = tagsStr.substring(inlineSourcesIdx).replace(/^SOURCES:\s*/i, "").trim();
          sources = sourcesInline
            .split(",")
            .map((s) => s.trim())
            .filter((s) => s.length > 0 && s.startsWith("http"));
          tagsStr = tagsStr.substring(0, inlineSourcesIdx).trim();
          // Remove trailing comma
          tagsStr = tagsStr.replace(/,\s*$/, "");
        }
        tags = tagsStr
          .split(",")
          .map((t) => t.trim())
          .filter((t) => t.length > 0);
        currentSection = null;
      } else if (upperLine.startsWith("COVER_IMAGE:")) {
        coverImage = normalizedLine.replace(/^COVER_IMAGE:\s*/i, "").trim();
        currentSection = null;
      } else if (upperLine.startsWith("YOUTUBE_VIDEOS:")) {
        if (currentSection === "content" && contentBuffer.length > 0) {
          articleContent = contentBuffer.join("\n").trim();
        }
        const ytStr = normalizedLine.replace(/^YOUTUBE_VIDEOS:\s*/i, "").trim();
        youtubeVideos = ytStr
          .split(",")
          .map((url) => url.trim())
        ...
        const sourcesStr = normalizedLine.replace(/^SOURCES:\s*/i, "").trim();
        sources = sourcesStr
          .split(",")
          .map((s) => s.trim())
          .filter((s) => s.length > 0 && s.startsWith("http"));
    ...
    // Sanitize tags: remove URLs, limit length, remove empty
    tags = tags
    ...
    // Sanitize sources: deduplicate and limit to 5
