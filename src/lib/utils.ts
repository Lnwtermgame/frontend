 ...
 * @param types - Array of product types with unitPrice
 * @returns Minimum price or 0 if no types
 */
export function getMinPrice(types: { unitPrice: number }[] | undefined): number {
  if (!types || types.length === 0) return 0;
  return Math.min(...types.map(t => t.unitPrice));
}
