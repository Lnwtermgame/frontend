...
export function DeliveryProvider({ children }: { children: ReactNode }) {
  ...
  const [deliveryItems, setDeliveryItems] = useLocalStorage<DeliveryItem[]>(
    "mali-gamepass-delivery-items",
    [
      {
        id: "dlv-001",
        orderId: "ord-001",
  ...
  // Mock delivery logs
  const [deliveryLogs, setDeliveryLogs] = useLocalStorage<
    Record<string, DeliveryLog[]>
  >("mali-gamepass-delivery-logs", {
    "dlv-001": [
      {
        id: "log-001-1",
        deliveryId: "dlv-001",
  ...
  const processDelivery = async (orderId: string): Promise<boolean> => {
    ...
    try {
      ...
      setError(
        `Failed to process delivery: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      return false;
    } finally {
      setIsPending(false);
  ...
  const cancelDelivery = async (deliveryId: string): Promise<boolean> => {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          "Only pending or processing deliveries can be cancelled",
        );
      }
      ...
      setError(
        `Failed to cancel delivery: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      return false;
    } finally {
      setIsPending(false);
  ...
  const resendDeliveryData = async (
    ...
    try {
      ...
      setError(
        `Failed to resend delivery: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      return false;
    } finally {
      setIsPending(false);
