...
class SupportApiService {
  ...
  async getFaqCategories(): Promise<ApiResponse<FaqCategory[]>> {
    const response = await supportClient.get<ApiResponse<FaqCategory[]>>('/api/support/faq/categories');
    return response.data;
  }

  async getFaqCategoryBySlug(slug: string): Promise<ApiResponse<FaqCategoryWithArticles>> {
    const response = await supportClient.get<ApiResponse<FaqCategoryWithArticles>>(`/api/support/faq/categories/${slug}`);
    return response.data;
  }
  ...
  async getFaqArticles(
    ...
    const response = await supportClient.get<ApiResponse<FaqArticleListItem[]>>(`/api/support/faq/articles?${params}`);
    return response.data;
  }

  async getFaqArticleBySlug(slug: string): Promise<ApiResponse<FaqArticle>> {
    const response = await supportClient.get<ApiResponse<FaqArticle>>(`/api/support/faq/articles/${slug}`);
    return response.data;
  }

  async searchFaqArticles(query: string, limit = 10): Promise<ApiResponse<FaqArticleListItem[]>> {
  ...
  async markArticleHelpful(articleId: string, isHelpful: boolean): Promise<FaqHelpfulResponse> {
  ...
  async getUserTickets(
    ...
    const response = await supportClient.get<ApiResponse<Ticket[]>>(`/api/support/tickets?${params}`);
    return response.data;
  }

  async createTicket(data: CreateTicketData): Promise<ApiResponse<Ticket>> {
    const response = await supportClient.post<ApiResponse<Ticket>>('/api/support/tickets', data);
    return response.data;
  }

  async getTicketDetail(ticketId: string): Promise<ApiResponse<TicketDetail>> {
    const response = await supportClient.get<ApiResponse<TicketDetail>>(`/api/support/tickets/${ticketId}`);
    return response.data;
  }

  async addReply(ticketId: string, data: TicketReplyData): Promise<ApiResponse<TicketMessage>> {
    const response = await supportClient.post<ApiResponse<TicketMessage>>(`/api/support/tickets/${ticketId}/reply`, data);
    return response.data;
  }

  async closeTicket(ticketId: string): Promise<ApiResponse<{ message: string }>> {
    const response = await supportClient.put<ApiResponse<{ message: string }>>(`/api/support/tickets/${ticketId}/close`);
    return response.data;
  }
  ...
  async updateTicket(ticketId: string, data: UpdateTicketData): Promise<ApiResponse<Ticket>> {
    const response = await supportClient.put<ApiResponse<Ticket>>(`/api/support/tickets/admin/${ticketId}`, data);
    return response.data;
  }

  async getTicketStats(): Promise<ApiResponse<TicketStats>> {
    const response = await supportClient.get<ApiResponse<TicketStats>>('/api/support/tickets/admin/stats');
    return response.data;
  }
