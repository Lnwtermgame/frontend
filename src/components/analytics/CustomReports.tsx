...
export default function CustomReports({
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "failed":
        return (
          <span className="px-3 py-1 text-xs bg-red-100 border-2 border-black text-red-700 font-medium">
            Failed
          </span>
        );
      default:
  ...
  const getFrequencyBadge = (frequency: string) => {
    const badges: Record<string, ReactElement> = {
      once: (
        <span className="px-2 py-0.5 text-xs bg-gray-100 border border-black text-gray-700">
          One-time
        </span>
      ),
      daily: (
        <span className="px-2 py-0.5 text-xs bg-purple-100 border border-black text-purple-700">
          Daily
        </span>
      ),
      weekly: (
        <span className="px-2 py-0.5 text-xs bg-blue-100 border border-black text-blue-700">
          Weekly
        </span>
      ),
      monthly: (
        <span className="px-2 py-0.5 text-xs bg-emerald-100 border border-black text-emerald-700">
          Monthly
        </span>
      ),
      custom: (
        <span className="px-2 py-0.5 text-xs bg-amber-100 border border-black text-amber-700">
          Custom
        </span>
      ),
    };
          ...
          <div className="space-y-4 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="flex items-center p-3 bg-gray-50 border-2 border-black hover:border-brutal-blue transition-colors">
                <input
                  type="checkbox"
                  className="h-4 w-4 mr-3 accent-brutal-blue"
                />
          ...
          {filteredReports.map((report) => (
            ...
            >
              ...
              <div className="flex flex-wrap justify-end gap-2 mt-4">
                ...
                <button
                  className="px-3 py-1.5 text-xs bg-gray-100 border-2 border-black text-gray-700 hover:bg-gray-200 flex items-center font-medium transition-colors"
                  style={{ boxShadow: "2px 2px 0 0 #000000" }}
                >
