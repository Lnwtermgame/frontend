...
export interface NotificationContextType {
  ...
  addNotification: (notification: { title: string; message: string; type: NotificationType; link?: string }) => void;
...
export function NotificationProvider({ children }: { children: ReactNode }) {
  ...
  const fetchNotifications = useCallback(async (page = 1, append = false) => {
    ...
    try {
      const response = await notificationApi.getNotifications(page, NOTIFICATIONS_PER_PAGE);
  ...
  useEffect(() => {
    if (!isAuthenticated || !token || typeof window === 'undefined') {
      console.log('[NotificationContext] WebSocket not connecting - auth:', isAuthenticated, 'token:', !!token);
      return;
    }

    // Prevent duplicate connection attempts from StrictMode
    if (wsSetupRef.current) {
      console.log('[NotificationContext] WebSocket setup already in progress, skipping');
      return;
    }
    ...
    const handleNotification = (notification: Notification, count: number) => {
      ...
      if (typeof window !== 'undefined' && 'Notification' in window && Notification.permission === 'granted') {
        try {
          ...
          console.error('[NotificationContext] Failed to show browser notification:', e);
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(() => {
      if (!isWebSocketConnected && !isLoading) {
        console.log('[NotificationContext] Polling for new notifications (WebSocket not connected)');
        fetchNotifications(1, false);
      }
    }, 30000);
  ...
  const addNotification = useCallback((notification: { title: string; message: string; type: NotificationType; link?: string }) => {
