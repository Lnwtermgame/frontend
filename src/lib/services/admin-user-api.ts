...
class AdminUserApiService {
  ...
  // Get single user
  async getUserById(userId: string): Promise<{ success: boolean; data: AdminUserDetail }> {
    const response = await authClient.get(`/api/admin/users/${userId}`);
    return response.data;
  }

  // Create new user
  async createUser(data: CreateUserData): Promise<{ success: boolean; data: AdminUser }> {
    const response = await authClient.post('/api/admin/users', data);
    return response.data;
  }

  // Update user
  async updateUser(userId: string, data: UpdateUserData): Promise<{ success: boolean; data: AdminUser }> {
  ...
  async updateUserRole(userId: string, role: 'USER' | 'ADMIN'): Promise<{ success: boolean; data: AdminUser }> {
    const response = await authClient.put(`/api/admin/users/${userId}/role`, { role });
    return response.data;
  }
