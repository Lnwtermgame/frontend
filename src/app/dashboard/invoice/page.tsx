...
import { FileText, Download, Eye, Calendar, Search, Filter, ChevronDown, CheckCircle, Clock, AlertCircle } from "lucide-react";
...
export default function InvoicePage() {
  ...
  const renderStatusBadge = (status: string) => {
    switch (status) {
      case "paid":
        return (
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-medium bg-mali-green/20 text-mali-green border border-mali-green/20">
            <CheckCircle className="w-3 h-3 mr-1" /> Paid
          </span>
        );
      case "pending":
        return (
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-medium bg-yellow-900/30 text-yellow-400 border border-yellow-400/20">
            <Clock className="w-3 h-3 mr-1" /> Pending
          </span>
        );
      case "cancelled":
        return (
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-medium bg-mali-red/20 text-mali-red border border-mali-red/20">
            <AlertCircle className="w-3 h-3 mr-1" /> Cancelled
          </span>
        );
      default:
        return (
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-medium bg-mali-blue/20 text-mali-text-secondary border border-mali-blue/20">
            {status}
          </span>
        );
    }
    ...
    <div>
      ...
      <div className="relative mb-6">
        ...
        <p className="text-mali-text-secondary text-sm relative">
          View and download your transaction invoices
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3 mb-6">
        <div className="relative w-full sm:w-80">
          <input
            type="text"
            placeholder="Search invoices or order ID..."
            value={searchTerm}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-mali-blue/10 border-b border-mali-blue/20">
              <tr>
                <th className="px-6 py-4 text-sm font-medium text-mali-text-secondary whitespace-nowrap">Invoice ID</th>
                <th className="px-6 py-4 text-sm font-medium text-mali-text-secondary whitespace-nowrap">Date</th>
                <th className="px-6 py-4 text-sm font-medium text-mali-text-secondary whitespace-nowrap">Order ID</th>
                <th className="px-6 py-4 text-sm font-medium text-mali-text-secondary whitespace-nowrap">Amount</th>
                <th className="px-6 py-4 text-sm font-medium text-mali-text-secondary whitespace-nowrap">Status</th>
                <th className="px-6 py-4 text-sm font-medium text-mali-text-secondary whitespace-nowrap text-right">Actions</th>
              </tr>
            </thead>
              ...
              {filteredInvoices.length > 0 ? (
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center">
                    ...
                    <p className="text-white text-lg font-medium mb-1">No invoices found</p>
                    <p className="text-sm text-mali-text-secondary max-w-md mx-auto">
